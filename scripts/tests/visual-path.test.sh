...
set -euo pipefail
cd "$(dirname "$0")/../.."
# 锚定脚本绝对路径（cd 之后捕获，供子 shell 在别的 cwd 下引用——不硬编码仓库位置）
SCRIPT="$PWD/skills/visual-preview/scripts/start-server.sh"
# Scenario: 特性上下文中的产物落位——只验证路径计算（--dry-run 不起服务器）
out=$(bash "$SCRIPT" --project-dir /tmp/vp-proj \
