#!/usr/bin/env node
import {recordExecution,verifyReceipt} from './lib/execution-evidence.mjs';
try{
  const [command,...args]=process.argv.slice(2),flags={},allowed={record:['--feature','--task','--phase','--attempt','--output'],verify:['--feature','--record','--candidate']}[command];
  const usage=m=>{throw Object.assign(new Error(m),{usage:true});};if(!allowed)usage('expected record or verify');
  ...
  else{if(argv.length)usage('verify takes no child command');result=verifyReceipt({feature:flags['--feature'],record:flags['--record'],candidate:flags['--candidate']});}
