...
import {git,localReference,activateReceipt,readBinding,clearReceipt,processMessage} from './lib/task-binding.mjs';
const usage=message=>{throw Object.assign(new Error(message),{usage:true});};
try{
  const [command,...args]=process.argv.slice(2),allowed={bind:['--plan','--task','--authority'],inspect:[],clear:['--plan','--task'],'prepare-message':['--file'],'check-message':['--file']}[command];
  if(!allowed)usage('expected bind, inspect, clear, prepare-message or check-message');
  ...
  if(command==='prepare-message'||command==='check-message')result=processMessage(repo,flags['--file'],command==='prepare-message');
