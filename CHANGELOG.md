...
- **skills**：新增 ddd-lifecycle 全流程开发规范
- **T05**：align public workflow guidance and preserve user glossary
- **T04**：bounded background research and source verification
- **T03**：controlled experiments and design handoff
- **T02**：show pending decisions and preserve user authority
- **T01**：integrate shared context and approved glossary persistence
- **T03**：save verified closure with previously committed metadata
- **T02**：clarify approved seam and repair scope with behavior protection
- **T01**：add evidence-based quick-fix diagnosis and escalation
- **T07**：complete requirement and context inputs
- **T06**：align refactor and group verification discipline
- **T05**：generate typed tasks and integration-group plans
- **T04**：integrate exclusive group execution and recovery
- **T03**：verify persisted Git checkpoints and evidence
- **T02**：implement v2 state and group readiness
- **T01**：validate integration group declarations
- **T04**：统一收尾编排与分发说明
- **T03**：通用派发完成条件
- **T02**：S/D 审查契约与 schema
- **T01**：共享模块判据与上游消费

### 🔧 修复 (Fixed)

- **T06**：sequence recovery by actual retry receipts
- **T06**：acquire external worker rule dependencies before research
- **T06**：compare semantic guarantees in final proposal summaries
- **T06**：verify first-question disclosure in the final message
- **T06**：verify final choices and preserve historical attribution
- **T06**：apply authorization and evidence checks to final outputs
- **qa**：preserve receipts and parse system text messages
- **T06**：bind source claims and preserve dispatch constraints
- **T06**：keep scope decisions independent from mechanisms
- **T06**：enforce clean inputs and shared rule acquisition
- **T06**：close reviewed clarification and source-loading gaps
- **T05**：align escalation summary with all approved signals
- close plan delivery and verify terminal integration archives
- preserve integration lock receipts and recovery checkpoints
- make integration group reference steps executable
- **T08**：preserve v1 activation and save blocked acceptance
- **T08**：make group lock and planning operations explicit
- **T08**：bind waiting evidence and clarify group checkpoints
- **review**：将审查片段预算调整为三十分钟
- **review**：完整声明工具契约并减少重复传输
- **review**：统一受控审查证据与有界恢复
- **review**：核对实际证据并避免虚构规范批准门
- **qa**：校准审查验收输入并核对真实工具回执
- **T05**：补强审查证据与独立执行回执

### 📝 文档 (Docs)

- activate status overview spec for planning
- close status overview design review
- design multi-worktree status overview
- **T07**：apply approved partial clarification supersession
- **T00**：declare exploring-clarifying coexistence
- **plan**：define exploring-clarifying implementation tasks
- **spec**：activate exploring-clarifying for planning
- **spec**：close independent exploring-clarifying review
- **spec**：define exploring and clarifying improvements
- **T06**：align final delivery records and roadmap capsule
- **T04**：synchronize public quick-fix descriptions and scenario mappings
- **T02**：classify existing behavior protection from observed baseline
- **T01**：record bounded decision fixture refinement
- **plan**：plan quick-fix diagnosis implementation and acceptance
- **spec**：activate approved quick-fix diagnosis design
- **spec**：record approved quick-fix diagnosis review
- **spec**：define quick-fix diagnosis and closure discipline
- checkpoint r8 acceptance results and remaining blockers
- **plan**：define plan decomposition implementation tasks
- 激活计划分解spec并登记取代预告
- 保存集成组设计独立审查结论
- 设计计划分解与多票集成组
- **T06**：锚定审查符合性交付并回写路线图
...
- **T04**：锚定共享编排与严格模型证据
- **T03**：锚定派发规则与验证记录
- **T02**：锚定任务提交与 CLI 红绿证据
- **T01**：锚定实际任务提交与验证结果
- **T00**：记录隔离基线与原始审查范围

### 🧹 其他 (Misc)

- finalize exploring clarifying recovery checkpoint
- **T07**：save final completion
- **T07**：anchor verified local delivery
- **T07**：save verified cleanup and retained evidence
- **T07**：record implementation cleanup
- **T07**：record empty fixture scope cleanup
- **T07**：record empty fixture scope cleanup
- **T07**：record one archived fixture cleanup
- **T07**：record one archived fixture cleanup
- **T07**：record one archived fixture cleanup
- **T07**：record one archived fixture cleanup
- **T07**：record one archived fixture cleanup
...
- **T07**：record one archived fixture cleanup
- **T07**：record one archived fixture cleanup
- **T07**：discharge one stopped CLI resource
- **T07**：discharge one stopped CLI resource
- **T07**：verify stopped owned CLI sessions
- **T07**：verify merged source and fresh process closure
- **T07**：record actual merge receipt
- **T07**：save final full-suite validation and closure checks
- **T07**：start verified local delivery
- **T06**：record verified acceptance completion
- **T06**：save complete Astra acceptance and independent closure
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T06**：freeze Astra acceptance manifest
- **T06**：pin authorized Astra runtime for complete acceptance
- **T06**：preserve r10 evidence and model decision checkpoint
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T06**：freeze recovery sequence candidate
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T06**：freeze worker authority candidate for uniform matrix
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T06**：freeze final-r8 acceptance candidate
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T06**：freeze final-r7 acceptance candidate
- **T06**：record default checks outside preserved raw replies
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T06**：freeze final-r6 acceptance candidate
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T06**：freeze final-r5 acceptance candidate
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T06**：freeze remaining diagnostic cases after host repair
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T06**：freeze final-r4 acceptance candidate
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T06**：freeze scope-independent final-r3 candidate
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T06**：freeze clean final-r2 candidate
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T06**：freeze candidate for complete model matrix
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T06**：start acceptance
- **T05**：save verified task completion
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **T05**：start task
- **T04**：save verified task completion
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register unexpected owned CLI sessions
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **T03**：start task
- **T02**：save verified task completion
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **T02**：start task
- **T01**：save verified task completion
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T01**：register short owned fixture scope and continuation
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **T01**：clarify fixture paths and preserve scoped evidence
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
- **qa**：register exploring fixture
...
- **qa**：register exploring fixture
- **T01**：start task
- **T00**：record verified isolation completion
- **T00**：verify real background capability and baseline
- **qa**：register exploring fixture
- **T00**：resume corrected capability preflight
- **T00**：preserve failed preflight and clarify retry input
- **qa**：register exploring fixture
- **T00**：bind actual isolated workspace
- **T06**：save final completion
- **T06**：anchor verified local delivery
- **T06**：record implementation cleanup
- **T06**：record one archived fixture cleanup
- **T06**：record one archived fixture cleanup
- **T06**：record one archived fixture cleanup
- **T06**：record one archived fixture cleanup
- **T06**：record one archived fixture cleanup
...
- **T06**：record one archived fixture cleanup
- **T06**：record one archived fixture cleanup
- **T06**：record actual merge receipt
- **T06**：save final179-test safety net and closure checks
- **T06**：start verified local delivery
- **T05**：record completed mandatory acceptance
- **T05**：save independently verified acceptance and complete evidence matrix
- **qa**：register owned quick-fix fixture
- **T05**：freeze independently reviewed acceptance candidate
- **T04**：save static synchronization completion
- **T04**：start public surface synchronization
- **T03**：save verified closure completion
- **qa**：register owned quick-fix fixture
- **T03**：record baseline evidence and green fixture ownership
- **qa**：register owned quick-fix fixture
- **T03**：start closure task and register fixtures
- **T02**：save verified completion
- **qa**：register owned quick-fix fixture
- **qa**：register owned quick-fix fixture
- **T02**：register green protection fixtures
- **qa**：register owned quick-fix fixture
- **T02**：start task and register bounded fixtures
- **T01**：save independently verified completion
- **qa**：register owned quick-fix fixture
- **qa**：register owned quick-fix fixture
- **qa**：register owned quick-fix fixture
- **qa**：register owned quick-fix fixture
...
- **qa**：register owned quick-fix fixture
- **qa**：register owned quick-fix fixture
- **T01**：start bounded diagnosis task
- **T00**：record verified isolation completion
- **T00**：save isolated baseline and execution authorization
- **T00**：bind actual isolated workspace
- **T09**：complete local delivery and roadmap closure
- **T09**：preserve complete remaining review fixtures before cleanup
- **T09**：prepare verified local delivery and resource archives
- **T08**：record completed acceptance checkpoint
- **T08**：close required model acceptance and evidence reconciliation
- preserve verified action chain and planning retry checkpoint
- preserve reviewed model runs and quota-blocked acceptance checkpoint
- **T08**：persist blocked recovery and coverage checkpoint
- **T07**：persist verified checkpoint
- **T06**：persist verified checkpoint
- **T05**：persist verified checkpoint
- **T04**：persist verified checkpoint
- **T04**：preserve model quota blocker and recovery checkpoint
- **T03**：persist verified checkpoint
- **T02**：persist verified checkpoint
- **T01**：persist verified checkpoint
- **T00**：establish plan decomposition baseline
...
- **T04**：贯通串并行消费与收尾审查
- **T03**：在设计与计划接口中传递获批 seam
- **T02**：补齐反模式、mock 分层与静态快检策略
- **T01**：统一 TDD 落点、红绿与例外定义
- **T06**：接通并发执行入口与共享交付
- **T05**：增加可选并发执行与中断恢复 skill
- **T04**：定义单票 implementer 与双交付通道
- **T03**：定义可选并发声明与恢复字段
- **T02**：核验实现结果与真实 Git 写边界
- **T01**：解析并发声明与写集合
...
- **T05**：保持票内验证与最终收尾的原时序
- **T05**：同步默认提示的纯重构路径
- **concurrent-execution**：preserve blocked claims before takeover
- **concurrent-execution**：enforce exact paths and advance verified baseline
- **T06**：保留执行入口的三件套说明
...
- **T01**：回写验证与实施提交
- **T00**：记录隔离与基线通过
- **plan**：编写 tdd-seam 七任务实施计划
- **spec**：激活 tdd-seam 并预告例外条款取代
- **spec**：记录 tdd-seam 独立审查通过
- **spec**：定义公共测试落点与红绿纪律
- **roadmap**：锚定并发执行交付并关闭资源台账
- **交付**：回写并发协议的部分取代与清理核验
- **progress**：记录 T07 完成
- **验收**：归档并发与中断恢复实测及独立复核
- **progress**：记录 T06 完成
...
- **progress**：记录 T03 完成
- **progress**：记录 T02 完成
- **progress**：记录 T01 完成并进入结果核验
- **T00**：保存隔离基线并开始并发声明校验
- **plan**：编写可选并发执行与中断恢复实施计划
- **spec**：激活并发执行契约并登记待取代条款
- **spec**：固定并发执行前模型与思考强度声明表格
- **spec**：增加任务边界并发切换与中断恢复协议
- **spec**：闭合并发锁身份与测试例外审查意见
- **spec**：定义可选并发执行与交付契约
- **T10**：完成交付对账、资源清理与 roadmap 状态回写
...
- **plan**：major-upgrade 实施计划落盘（28 任务：vendored 基建/平台诊断/anysearch/产物组织/澄清纪律/规范载体/plan 双形态）
...
- **spec**：采纳审查发现——资源登记纪律按形态分流重述、补 4 个 Scenario 与矩阵行、MCP 移除对象修正
...
- **spec**：取代分流修正——clarifying 降级分面共存，resource-ledger 部分取代改为实文 Requirement 标题
...
- **spec**：重大升级整合设计落盘（多平台适配 · MCP 清零 · 工作流增强）
...
- **explore**：降级重试纪律调整——每层最多 2 次尝试、间隔 30s 留瞬时故障恢复窗口,确定性失败(CLI 缺失/配额尽)即刻降级
...
- **explore**：外部探索优先 AnySearch 并内置智能降级——external-resource-explorer 集成内嵌 CLI 用法与单向降级判定(runtime/网络/配额),降级链权威定义与派发文档同步更新
...
- **roadmap**：README 管线图补 roadmap 外环与产物清单
...
- **roadmap**：executing-plans 收尾闭环——合并后回写 roadmap 子项目状态 delivered，全部交付翻 done，总结提示续接下一个子项目
...
- **roadmap**：writing-plans 范围检查改造——多子系统/阶段化 spec 首选回炉分解，不回炉则只写第一份计划、剩余范围登记 roadmap；交接话术带 roadmap 进度锚点
...
- abuse
- **reconciliation**：补齐 7.10.0/7.11.0 遗漏的 SKILL 正文（rtk 代理吞掉 commit -a，内容未暂存）
...
- **reconciliation**：executing-plans 阶段5重写为例外驱动统一停顿门 + 阶段6加对账落盘与 sync_commit 锚定
...
- **reconciliation**：spec 模板 sync_commit 改为收尾锚定语义 + 交付偏差原位标注惯例
...
- 产物统一收纳项目根 .spec-dev/ 目录，docs/ 历史位置默认自动迁移
...
- Skill 管线段补 quick-fix 旁路入口与三入口分工说明（双版）
...
- **quick-fix**：新增 quick-fix skill 实施计划（任务 0-8）
...
- **quick-fix**：spec 定稿转 status: active，激活漂移守卫锚点
...
- **quick-fix**：spec 对抗审查修正——反查 glob 与守卫三条对齐 + 契约不变提交显式 SPEC_DEV_GUARD=off+trailer
...
- **quick-fix**：新增 quick-fix skill 设计 spec 与 ADR-0002
