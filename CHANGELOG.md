...
- 新增共用任务写集合、版本绑定和持久 `Spec-Task` 关联；编辑、暂存、提交历史及 push/CI 按各自视图核验，合法实施无需伪造 spec 修改。
- 恢复时优先处理当前任务；旧计划按明确任务局部补齐范围与收尾步骤，保留已完成提交和证据。
- 将 final 全量验证前移至独立审查与验收之前，消除收尾依赖循环；后续修复必须补验、复审并更新对账。
- 统一真实执行回执、内容范围核验和审查导入；本地原件、失败尝试及原生审查资料经核验转存后再清理，最终 merge/squash 保留来源历史与目标映射。
- 统一大变更的 critic 义务与分档验收审计，修复历史对象缺失误放行、已有消息 hook 安装及终止确认边界。
- 为需求路由、小修判断和执行恢复增加流程图，精简文案并统一正文、参考细则与技能摘要。

验证：实现版本全量测试 455/455 通过；真实模型评估及 Mermaid 图形渲染未验证。
...
- 统一 DDD 文档位置与保存授权，共用词汇表及 ADR 规范。
- 改进容量驱动派发、证据驱动契约判断、项目环境就绪与有效失败测试说明。
- 统一验收范围和浏览器 MCP 配置说明，精简探索报告及 TDD 重复文案。

验证：全量测试 253/253、必需模型冒烟 10/10 通过；完整夜间模型评测未运行。

## [8.5.0] - 2026-09-10

- 新增多工作区计划状态总览，汇总任务进度、执行状态及交付证据。
- 修复未知任务状态被误判为不存在、旧计划来源丢失及 Unicode 边界处理问题。
- 支持依据交付和后续验证证据回填旧计划进度，并区分历史过程与最终结果。

## [8.4.0] - 2026-09-10

- 新增 `ddd-lifecycle`，覆盖领域建模、统一语言、限界上下文、六边形架构及 ADR。
- 增强探索与澄清：复用上下文和已批准决定，支持受控实验、有界后台研究及一手来源核验。
- 改进小修流程：基于复现证据诊断根因，明确测试落点、修复范围与升级条件。
- 引入类型化任务和集成组计划，支持分票待验、统一验证、独占执行及检查点恢复。
- 统一独立审查、实现符合性检查与覆盖对账，强化真实执行回执和失败恢复。
