...
参数格式：`[layer1|layer2|layer3|all] <target-description>`（前缀可选，自然语言触发时按意图推断）。

## 参数解析（三级：显式前缀 > 意图推断 > 询问）

解析 `$ARGUMENTS`：

1. **显式前缀**（兼容老用法）：`layer1|layer2|layer3|all <描述>` → 按前缀执行
2. **无前缀 → 意图推断**：
   - "补充/编写 E2E、回归测试" → Layer 1
   - "验收/检查/看看这个页面|功能" → Layer 2（发现失败项时自动追加 Layer 3）
   - "诊断/排查/为什么…不工作" → Layer 3
   - "全面测试/完整验收" → all
3. **意图模糊**（如只给了一个 URL、无动词）→ 询问用户测试目标与期望层级
...
- 每条用例至少包含一个会因功能破坏而失败的业务断言（数据变化、URL 跳转、错误提示），禁止仅断言元素可见——恒真/过弱断言让测试形同虚设
...
# 证据审计（独立子代理，只读已落盘证据、不操作浏览器——不占浏览器会话，可与上面的复核并行发起）：
...
#   动机：AI 验收的主导失败模式是假阳性，执行者判读自己的证据存在系统性确认偏差——
#   重执行复核只覆盖 fail/warn 方向，pass 方向由主体独立的审计兜底
# 重执行式 pass 复核仍仅在用户要求"严格验收"时抽查 2 项（占浏览器会话，成本控制）
...
| 检查项 | 结果 | 证据 | 复核 | 备注 |
|--------|------|------|------|------|
| 购物车数量修改后小计实时更新 | ✅ | snapshot: 小计 ¥58→¥116 | 审计维持（证据含修改前后对比） | 数量 1→2 |
...
| 响应式布局（桌面端） | ✅→未验证 | snapshot: 1280px 布局正常 | 审计降级（证据为首屏截图，未覆盖布局完整性） | 需补证 |
...
结果只有四种：`✅ 通过`（有证据且经审计确认）、`⚠️ 警告`（有证据）、`❌ 阻塞`（有证据）、`未验证`（无证据、证据不支撑结论、或未执行——不允许无证据的 ✅）。复核列记录：fail/warn 项=重执行复核结论（维持/降级/翻案 + 第二份证据引用）；pass 项=证据审计结论（审计维持/审计降级 + 缺口说明）。
...
**根因必须经假设验证后才能写入报告**：按"若根因成立，做 X 应观察到 Y"做一次可观察的预测并实际验证（读源码确认对应行确实缺失该逻辑 / 复现一次确认现象与根因吻合）。验证不一致 → 回到 Step 1 重新分类诊断——未经验证的推理结论会让用户按错误方向修复并掩盖真因。
...
**验证**: 预测"点击提交后按钮无 disabled 属性变化"——snapshot 复核一致；源码确认 42 行无 isSubmitting 绑定
**修复建议**:
...
**输出约定**：若由 spec-flow accept（或其他工作流）触发，报告文件与截图落盘到调用方指定的 evidence 目录（如 `.specs/active/<id>/evidence/browser/`）并回传文件路径——只打印在对话中的报告无法被后续验收编排引用。直接面向用户时输出在对话中即可。
