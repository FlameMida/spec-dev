...
// Per-session secret key. The preview is reachable by any local browser tab
...
<html>
<head><meta charset="utf-8"><title>Visual Preview</title>
<style>
...
function bootstrapPage(key) {
  ...
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Opening Visual Preview</title></head>
<body>
<script>
...
function brandMarkup() {
  ...
  return '<div class="brand"><span class="brand-copy">Visual Preview v' + version + '</span></div>';
}
...
function maybeOpenBrowser() {
  ...
  browserOpened = true;
  if (!process.env.BRAINSTORM_OPEN) return; // opt-in: only after the user approves the preview
  if (HOST !== '127.0.0.1' && HOST !== 'localhost') return;
  if (clients.size > 0) return; // the user already opened it
  const url = previewUrl(); // must carry the key or the gate 403s it
