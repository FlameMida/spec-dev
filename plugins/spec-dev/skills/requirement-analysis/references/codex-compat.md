...
用 `spawn_agent` 以**不继承主会话上下文**的方式派发（审查者不应继承主会话立场；新版参数 `fork_turns: "none"`，旧版 `fork_context: false`），提示词按 [spec-reviewer-prompt.md](spec-reviewer-prompt.md) 模板构造，`wait_agent` 收集结果。
