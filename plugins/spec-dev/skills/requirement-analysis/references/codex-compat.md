...
**阶段 6 路径 3（计划拆分落盘）的任务注册**：用户选择立即实施时，用 `update_plan` 将 `tasks.md` 任务清单逐项注册（每任务一项，命名 `T{n}: 任务名`）；实施期间先更新三件套文档状态，再同步 plan 状态——文档是持久层，顺序颠倒会让中断恢复点失真。细则见 [plan-handoff.md](plan-handoff.md)。

**任务管理功能不可用时**：降级为纯文本阶段进度说明，继续执行主流程，并向用户说明情况。

---

## 提问规范（阶段 4 澄清、阶段 6 路径确认、阶段 8 审查处理）
...
阶段 6 的路径确认与阶段 8 的审查处理同样适用：直接发送确认问题并等待用户明确回复后再继续。

**阶段 6 路径菜单的 Codex 形态**：菜单仅提供路径 1（直接实施）与路径 3（计划拆分落盘），隐藏路径 2（与 codex 计划讨论）——路径 2 的价值在于引入第二个独立模型的评审视角，Codex 环境自身就是 codex，自我评审没有独立性收益。菜单以对话消息呈现数字选项并保留修改意见入口，用户对计划给出修改意见时回到计划修订、修改后重出菜单。
