...
路径 2/3 的执行细则（codex 命令序列、三件套生成与双轨同步规则、与 spec-flow 的边界）见 [plan-handoff.md](references/plan-handoff.md)；Codex 环境下菜单仅提供路径 1/3（理由与形态见 [codex-compat.md](references/codex-compat.md)）。

**升级出口（出菜单时附注）**：满足任一条件时，在菜单中附注建议切换 spec-flow：
...
- [计划交接指南](references/plan-handoff.md)
...
- [计划拆分三件套模板](assets/plan-split-templates.md)
