...
description: 执行实施计划——当已有 writing-plans 产出的实施计划（特性目录 docs/ 下 plan/ 子目录的 *-plan.md）、准备动手实现时使用。主线程在隔离 worktree 中逐任务执行（TDD + 每任务提交 + spec 自检），全部完成后编排多维对抗代码审查（code-reviewer 子代理只读分析），UI 变更触发 browser-qa 验收，最终合并总结。不适用于没有书面计划的即兴改动。
---
...
**核心范式**：**主线程干活、子代理只读分析**——实现代码由主线程编写（保证上下文连续与契约一致），子代理只承担审查等只读任务。
...
1. **载入并批判性审阅计划** — 有疑虑先向用户提出
2. **隔离工作区** — 执行计划的任务 0（纪律遵循 using-git-worktrees）
3. **逐任务执行** — TDD + 每任务提交 + spec 自检，连续执行
4. **收尾审查** — 多维 fan-out + 对抗复核 + completeness critic；UI 变更加 browser-qa
5. **审查处置** — 征询用户，修复确认项
6. **合并与总结** — worktree 合并回来源分支，输出总结
...
3. 无疑虑则把计划任务注册进任务管理（每任务一条，`T{n}: 任务名` 命名），开始执行
...
任务 0 已在阶段 2 完成；对计划中任务 1 起的每个任务，按序：
...
**偏差处理**：执行中发现计划错误（接口对不上、步骤缺失）→ 小偏差就地修正并在最终总结中记录；影响后续任务的契约级偏差 → 停下向用户确认。
...
- **browser-qa**：变更涉及 UI 时，触发 browser-qa skill 做 Layer 2 验收（验收项引用 spec 中标注的 Scenario 与计划验收点）
...
1. 审查通过后：worktree 分支合并回来源分支，删除 worktree（原生工具建的用原生方式退出）
2. 输出总结：
   - **成果清单**：完成的任务、创建/修改的文件
