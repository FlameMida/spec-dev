...
- **最终题目核对**——构造本次实际要发出的题目后，按每个答案会落实的用户裁决核对：同一份需求或许可政策不能同时命中两项；细则留待以后时，说明当前哪些边界已决定、哪些仍未决定，不用含糊的后定项充当另一选项。每项只锁定当前决策，不代为确定清单中的其他未决项。不满足则先改写最终选项再发送；核对是内部步骤，不额外向用户提问或要求工件。
