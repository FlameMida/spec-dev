...
- **选择题优先**——每题尽量给 2-4 个互斥的具体选项，**推荐项放首位并说明理由**。只列足够区分当前决策的最少选项，能二选一就不给多余分支；扩展范围已经包含原范围时，不另列“两者都要”。选项只比较同一个决策维度；若一种实际选择能同时落入两项，先修正边界。上游保证范围与下游实现机制等不同维度分列为依赖事项，不混成一组选项（Claude Code 用 `AskUserQuestion`；没有合适选项的开放式问题用对话直接问）
