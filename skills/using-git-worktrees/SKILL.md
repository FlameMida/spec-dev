...
（声明为空 → 跳过测试、直接报告就绪并注明"纯文档特性，基线测试跳过"；声明命令报错 → 改跑该计划列出的自有测试文件，注明声明已失效、建议修订计划，不回退全量）；计划无该节 → 跑其任务文件中出现的测试文件；无计划 → 运行项目完整测试命令（`npm test` / `cargo test` /
`pytest` / `go test ./...`）：
...
| 创建遇权限错误 | 沙箱降级，原地工作 |
| 计划声明了「相关测试范围」 | 基线只跑声明范围（空→跳过并注明；声明失效→自有测试，不回退全量） |
| 基线测试失败 | 报告 + 询问 |
| 无依赖清单文件 | 跳过安装 |
