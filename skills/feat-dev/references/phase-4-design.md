...
#### 项目规范要点
${如果有 AGENTS.md 或 CLAUDE.md,列出关键规范}
...
**使用条件**：
- 仅在主进程已完成核心分析、且需要补充方案细节时使用
- 如果下一步强依赖这份结果，Claude 与 Codex 都只启动 1 个阻塞型子代理
- Codex 可直接使用子代理；若当前阶段更适合主进程整体判断，也可以不委派
...
1. **探索代码库** - 深入探索相关代码
2. **数据库设计** - 实体定义、关联、索引，符合项目规范
3. **API 端点设计** - REST 端点、请求/响应，符合项目规范
4. **服务层架构** - 服务接口、业务逻辑、依赖注入，符合项目规范
...
Codex:
- spawn_agent(agent_type="default", fork_context=true, message="[同样的结构化上下文和任务说明]")
- 然后立即 wait_agent
```
...
⚠️ **不使用主进程 ultrathink**（子代理会各自进行深度分析）

⚠️ **并行执行要求**：
- **必须在单个响应中**发起 2 个并行子任务；只有明确需要第三种备选方案时才扩到 3 个
- Claude Code：每个 Task 设置 `run_in_background: true`
- Codex：使用 `spawn_agent(agent_type="default", fork_context=true, ...)`
- 然后收集每个子代理的结果（Claude 用 `TaskOutput`；Codex 用 `wait_agent`）

**方案分工**：
- 方案 1: 最小改动方案 - 快速交付，最小化风险
- 方案 2: 清晰架构方案 - 追求最佳实践和长期可维护性
- 方案 3: 实用平衡方案 - 在速度和质量之间找平衡（通常推荐）

**并行调用模板**：

每个子代理使用上面的**结构化上下文模板**传递信息，并且各自负责一个备选方案：
...
Codex:
- 对每个方案分别 spawn_agent
- 汇总阶段使用 wait_agent 收集结果
```

**失败处理**：
- 任一方案子代理未返回完整方案、关键文件或权衡理由，视为失败
- 失败后先缩小 prompt 重试 1 次
- 仍失败则放弃并行多方案，主进程收敛为单方案分析
