...
- exa → 网页搜索
- sequential-thinking → 结构化计划或手动分步分析
...
### AGENTS.md / CLAUDE.md 文件缺失

**症状**：项目根目录没有 `AGENTS.md` 或 `CLAUDE.md` 文件
...
3. 在总结阶段建议创建 `AGENTS.md` 或 `CLAUDE.md`

### AGENTS.md / CLAUDE.md 规范不明确

**症状**：`AGENTS.md` 或 `CLAUDE.md` 存在但描述模糊
...
2. **优化并行策略**：
   - 阶段 2：默认 1-2 个 exploration 子代理，最多 3 个
   - 阶段 6：默认 2 个 review 子代理，最多 3 个
   - 不要启动过多子代理
...
4. 如果降级，使用 `update_plan` 或显式分步规划进行更详细的分析
...
**症状**：Claude Code 的 Task 或 Codex 的 spawn_agent / wait_agent 执行失败、超时、结果为空

**解决方案**：
1. 先确认任务是否过大，收敛成一个更清晰的主题或问题
2. 减少并发数量，优先保留最关键的 1-2 个子任务
3. 补足上下文：需求摘要、相关文件、输出格式、完成标准
4. 确认子代理边界是否清楚，但不要压缩到失去分析空间
5. 重试 1 次；仍失败则由主进程直接完成
