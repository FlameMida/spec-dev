...
progress 的 delivery 由 writing-plans 定义，主线程独占写入。implementing、awaiting_merge、merged、completed 只记录事实，不构成证据。独立 final、全局审查、矩阵验收与对账通过后才可按授权标 ready；PR 仍 open 时 state=awaiting_merge，最终任务仍 in_progress，roadmap 仍 in-progress，sync_commit 不锚定。可以结束当前会话，保留恢复所需分支、状态与证据。
...
完成前按 writing-plans 的 delivery 字段与实际命令保存 source_tip/source_tree、保留 history_ref、原始交付回执和必要补验，再调用 verifyDelivery。普通 merge 验证来源 ancestry；最终 squash 验证保留来源历史与目标映射，票级集成不放宽。证据 transfer 到不随原 worktree 删除的存活目录，逐字节核对后才清理；失败保留原件与已完成条目，不改历史 cwd。
