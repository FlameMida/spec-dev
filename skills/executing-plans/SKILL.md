...
5. **审查处置** — 征询用户，修复确认项
6. **合并与总结** — 执行计划的最终任务（合并与清理），输出总结

## 阶段 1：载入并批判性审阅计划

1. 读取计划文件（`docs/YYYY-MM-DD-<feature>/plan/<feature>-plan.md`）与同特性目录的 `spec/<feature>-design.md`
...
按严重性分组展示 confirmed 发现 + 覆盖声明，**征询用户处理方式**——哪些值得修、按什么顺序修是用户的优先级决策，不自动修复。用户确认后在 worktree 内修复（修复本身同样走 TDD：先写复现测试），修复后受影响维度复审一次。

## 阶段 6：合并与总结

1. 审查通过后：执行计划的最终任务（全量验证 → 合并回来源分支 → 清理 worktree 与分支；原生工具建的隔离用原生方式退出）；计划缺最终任务（旧版计划）时按同等步骤手工收尾
2. 输出总结：
   - **成果清单**：完成的任务、创建/修改的文件
