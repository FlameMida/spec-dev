...
**存量单文件的轻量恢复（兼容分支）**：单文件计划无 progress.yaml；先核对已有绑定、明确 task key 及授权/执行记录，定位本次任务。未勾选步骤只提供候选，不能把首项当授权；多个未完成任务且无法唯一定位时报告具体缺口，不猜测。复选框与实际提交、验证原件逐项对照；提交存在不代表验证和状态保存已完成。补写当前/未执行范围后，按稳定 TNN 绑定，保留原历史。
