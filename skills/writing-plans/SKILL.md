...
**不变式：一次只写一份计划，不为未实施的后续阶段预写计划**。计划要求每步含关键 diff 片段与精确路径，后续阶段的代码建立在前一阶段尚不存在的产物上——现在写出来必然失效。后续子项目在前置交付后按 roadmap 续接（executing-plans 收尾会核对 roadmap 并提示下一个）。
...
拆任务时检查哪些任务可以并发：核对真实依赖、修改文件和共享资源；符合条件的按[并发声明](references/parallel-declaration.md)生成声明，不能并发的在计划中简述原因。默认仍串行，只有用户明确选择才启用并发。
...
- "为上述代码写测试"（没有实际测试代码）
- "类似任务 N"（把该任务自己的片段写出来——工程师可能乱序阅读任务）
- 只说做什么不给怎么做的步骤（涉及代码的步骤必须有片段或新旧对照）
- 引用任何任务中都未定义的类型、函数、方法

## 牢记

- 永远给精确文件路径
- 每步给关键片段——改代码的步骤必须展示新旧对照或新增代码，不抄整文件
- 精确命令 + 预期输出
- DRY、YAGNI、TDD、频繁提交
