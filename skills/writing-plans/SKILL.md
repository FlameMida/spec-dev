...
所有实施任务与验收任务（如有）之后，固定以下述任务收尾（编号顺延全局任务号）——与任务 0 首尾对称，worktree 从建立到合并的生命周期在计划文档内闭合。使用 executing-plans 编排执行时，本任务不参与阶段 3 连续执行，推迟到收尾审查处置完成后运行：
...
任务 0 未由本计划建立 worktree（此前已在隔离环境、原生工具建立、或降级原地执行）→ 只执行步骤 1 与步骤 4，步骤 2-3 交回原有隔离机制收尾并注明。
