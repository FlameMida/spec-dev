...
独立有界的只读子题可委派后台调查，主线程继续不依赖结果的讨论；依赖该事实前必须回收并核对来源、覆盖与缺口。派发前先实际读取 [exploration-patterns.md](../requirement-analysis/references/exploration-patterns.md) 的定义加载与派发要求，插件根解析和失败接管也按该单点执行，外部来源纪律以 agents/external-resource-explorer.md 为准；非自动加载环境由派发路径要求先读取定义。只有根本没有合法的代理派发能力时，才直接由主线程按同一 reference 的定义读取要求接管。已有子题回报未完成、且仍能新派发时，先在原每次限制内做一次实质缩域的新派发；仍失败再接管。没有消息续接 API 不等于不能新派发，不能据此跳过这一次重试。缺口不伪造已完成。外部研究派发按该 reference 携带完整工具优先级和 CLI 实路径；离线限制另行说明，不能替代提醒。后台不能写实现、自动落盘笔记或代用户裁决。
