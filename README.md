...
仓库采用扁平结构：仓库根即插件根，Claude Code 与 Codex 两个 marketplace 清单都指向 `./`。Codex 清单中插件源使用 `{"source": "url", "url": "./"}` 写法（`source.path` 为 `./` 时会被 Codex 规范化为空路径而在 `/plugin` 中被过滤，`url` 写法无此问题）。

如果本地已经添加过旧版本 marketplace，请在新版本发布后执行 `codex plugin marketplace upgrade spec-agent-skills`；本地开发验证可先移除旧源，再添加当前仓库路径。

marketplace 清单的顶层 `name` 是 `spec-agent-skills`（marketplace 标识，用于 `upgrade` 与 `插件名@marketplace` 引用），其中的插件条目 `name` 是 `spec-dev`（插件标识，用于 `codex plugin add spec-dev@spec-agent-skills`）：
