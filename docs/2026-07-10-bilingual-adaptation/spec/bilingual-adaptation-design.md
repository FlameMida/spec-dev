...
- **skills/**：8 个 SKILL.md（协议块 + frontmatter description）、8 个 agents/openai.yaml（default_prompt）、requirement-analysis 与 acceptance-qa 的产物模板、visual-preview 的 preview-guide、requirement-analysis 与 exploring 与 acceptance-qa 的 trigger-evals.json
...
- **7 处固定话术清单**（供实施定位）：requirement-analysis L198（spec 审查门）、writing-plans L12 与 L234-236（开场/交接）、executing-plans L14（开场）、using-git-worktrees L35-36（就绪报告）与 L40（征询建 worktree 同意）、visual-preview L14（JIT 提议）
...
含 trigger-evals.json 的三个 skill（requirement-analysis、exploring、acceptance-qa）SHALL 各在现有英文用例基础上扩充至至少 2 条英文触发正例与 1 条英文边界负例，格式沿用现有可判定结构（requirement-analysis 与 exploring 现各有 1 条英文正例 t6，acceptance-qa 已有多条英文用例——本条为增补而非从零建；visual-preview 无 trigger-evals.json，其触发为流程内 JIT 提议型，不在本条范围）。
...
- **WHEN** 以三个 skill（requirement-analysis、exploring、acceptance-qa）现存中文 trigger-evals 用例回归
- **THEN** 全部用例判定结果与改动前一致

### Requirement: 产物模板双语指引（改：中文注释 → 双语指引+英文结构）

spec-template.md（`skills/requirement-analysis/assets/`）与 acceptance-report.md（`skills/acceptance-qa/templates/`）SHALL 在顶部含双语填写指示（内容跟随对话语言）；填写说明注释 SHALL 为中英对照；结构标签、表头 SHALL 统一英文；占位符名 SHALL 为英文。preview-guide.md（`skills/visual-preview/references/`）SHALL 声明示例中文文案仅为结构示意、实际 mockup 文案用对话语言。
...
> **验收维度说明**：本特性属"插件指令与文档的元改造"，天然落在 acceptance-qa `references/acceptance-matrix.md` 的 7 维产品验收枚举（unit/integration/e2e/visual/a11y/perf-web/perf-api）之外。下表「维度」列中 `evals`、`docs`、`ai-acceptance` 为本特性自定义维度；writing-plans 按「执行方式」列路由任务、acceptance-qa 按「验收证据」列方法执行，二者均不硬依赖标准维度名。`integration` 为标准枚举内值。
