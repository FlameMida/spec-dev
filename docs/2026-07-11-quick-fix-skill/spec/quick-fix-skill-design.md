...
quick-fix 在定位根因时 SHALL 用嫌疑改动文件反查仓库内 `status: active` spec 的 `covers` glob（Grep `docs/**/spec/*-design.md` 与 `.specs/**/*.md` 并解析 frontmatter），把命中 spec 的相关 Requirement/Scenario 读入上下文；反查 SHALL NOT 依赖目标仓库安装过 guardrail 脚本。
...
契约变更（单 spec 范围内）时，quick-fix SHALL 同步该 spec 的相关 Requirement/Scenario 小节、将 spec 增量展示给用户过目，并把 spec+代码+测试放入同一提交；契约不变时 SHALL 不改 spec，提交信息留 `Spec-Guard: off <原因>` trailer。
...
- **WHEN** 修复完成提交
- **THEN** spec 未被改动，commit message 含 `Spec-Guard: off <原因>` trailer
...
- 编辑期 hook 拦截契约不变修复：显式用户交互 + `SPEC_DEV_GUARD=off` + trailer（见 Requirement「契约分流提交」）
