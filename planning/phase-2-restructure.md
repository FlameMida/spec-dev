...
- **改动步骤**:
  ...
  5. 目标行数 **< 300 行**。
- **验收标准**:
  - [ ] `wc -l skills/requirement-analysis/SKILL.md` < 300
  - [ ] 9 个阶段的业务要点无丢失（对照旧版逐阶段核对）
  - [ ] 全文新增/保留的"必须"类指令均带一句 why 或降级路径
  - [ ] `node scripts/validate-skills.mjs` 通过
  - [ ] 在一个真实中等复杂度需求上试跑，流程可走通
...
- **验收标准**:
  - [ ] 检查清单生成是"定制 + 通用筛选"两段式
  - [ ] 报告模板含证据列，且有「无证据 → 未验证」的降级规则
  - [ ] 在示例页面试跑一次 Layer 2，产出报告每项均有证据引用
