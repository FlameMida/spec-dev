...
- **状态**: ✅ 完成（2026-06-12）　**预估**: 2h　**依赖**: T3.1、T3.2、T2.9（证据强制已落）
...
- **验收标准**:
  - [x] Layer 2 为伪代码形态；串行约束在编排开头与"不要做的事"两处显式声明
  - [x] fail/warn 必复核、pass 默认不复核的分级规则明确（严格验收时抽查 2 项 pass）
  - [x] 契约 JSON 落盘 + 校验步骤存在（validate-output.mjs browser-check-items，失败补全一次、再失败标 unverified）
  - [x] 在示例页面试跑：制造一个真实 fail 项，验证复核环节产出第二份证据（实测：演示登录页故意无 loading 状态 + 800ms 延迟反馈 → 检查项判 fail [证据1: snapshot 按钮无 disabled] → 不信任视角复核：程序化 0-400ms 连续采样按钮状态，复现成功维持原判 [证据2: 采样数据排除时序偏差假设] → 契约 JSON 落盘校验 PASS）
