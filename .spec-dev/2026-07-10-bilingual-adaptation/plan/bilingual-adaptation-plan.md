...
> **执行方式**：使用 spec-dev 的 executing-plans skill 逐任务执行本计划；无该 skill 的环境直接从任务 0 起按序执行至最终任务。步骤用复选框（`- [x]`）语法跟踪；脱离项目携带时连同特性目录（含 spec）整体带走。
>
...
- [x] **步骤 2：3 个 agent 正文 H1 标题之后插入语言条款（前后空行）**
...
- [x] **步骤 3：4 个 frontmatter description 替换为拼接版**
...
- [x] **步骤 2：替换 8 个 SKILL.md 的 description**
...
- [x] **步骤 3：8 个 openai.yaml 更新 short_description 与 default_prompt**
...
- [x] **步骤 3：acceptance-report.md 全文替换为**
...
- [x] **步骤 4：preview-guide.md** 在 H1 之后插入（前后空行）：
...
- [x] **步骤 2：追加条目（JSON 数组尾部，保持既有缩进风格）**
...
- [x] **步骤 2：英文化 guardrail/README.md**
...
- [x] **步骤 2：CLAUDE.md.snippet 标记块内改为英文节+中文节并列**
...
- [x] **步骤 4：session-context.mjs 输出双语**
...
- [x] **步骤 2：按映射表逐条替换（格式 `English / 中文`，中文侧一字不改）**
