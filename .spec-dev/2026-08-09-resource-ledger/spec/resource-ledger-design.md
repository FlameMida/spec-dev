...
acceptance-qa 环境检测发现运行中的可复用服务实例时，执行者 SHALL 按序判定：能创建隔离单元则复用该实例并仅登记自建隔离单元；无法隔离则新建专用实例并整体登记；两种决定均 SHALL 向用户声明一句；对共享实例 SHALL NOT 执行破坏性操作（DROP DATABASE、清空数据卷、停止/重启非自建容器）。
