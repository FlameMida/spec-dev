...
quick-fix 在定位根因时 SHALL 用嫌疑改动文件反查仓库内 `status: active` spec 的 `covers` glob，反查的 spec 发现范围 SHALL 与守卫 `loadActiveSpecs` 逐字对齐（五条 glob：`.spec-dev/**/spec/*-design.md`、`.spec-dev/**/*-design.md`、`docs/**/spec/*-design.md`（历史位置）、`docs/**/*-design.md`（历史位置）、`.specs/**/*.md`），把命中 spec 的相关 Requirement/Scenario 读入上下文；命中 `docs/` 历史位置的 spec-dev 产物时 SHALL 默认先自动迁移到 `.spec-dev/` 并单独提交，再继续修复；反查 SHALL NOT 依赖目标仓库安装过 guardrail 脚本。

#### Scenario: 命中 active spec

- **GIVEN** 嫌疑文件 `src/export/pager.ts` 被 `.spec-dev/2026-06-01-export/spec/export-design.md`（status: active，covers 含 `src/export/**`）覆盖
