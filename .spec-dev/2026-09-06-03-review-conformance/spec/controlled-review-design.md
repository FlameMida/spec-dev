...
> **Superseded-pending (2026-09-12)** — 本 spec 的「Requirement: 证据由宿主管理」将被 .spec-dev/2026-09-12-01-workflow-slimming/spec/workflow-slimming-design.md 部分取代（待其交付）；新工作以新 spec 为准，本 spec 仍描述当前已实现行为。
...
本次明确扩展原设计第44/348行的非目标，允许生产审查运行器；原28个Scenario和审查判断仍有效。review-findings现有字段、类别与通用validator不变，新的运行记录是独立外层协议。执行期间T05保持in_progress，T06在本修正与原验收完成后开始；现T05/T06已完成本地交付，最终状态见plan/progress.yaml。旧50条模型证据全部保留，不改写历史结果。
