...
**接口**：
- 消费：review-findings 新类别 Spec符合性；description/coverage_note 语义；CLI exit/JSON；S01-S05/S13/S19-S21。
- 产出：exploration-patterns 完成条件/排除项单点；保留时效和重试；S24-S25。
...
import json
batches = [('skills/requirement-analysis/evals/evals.json', [{'id': 'review-conformance-s24', 'prompt': 'S24 泛泛任务改成有界派发\nGIVEN 待派发内容只有「检查审查纪律」\nWHEN 按通用派发纪律准备输入', 'expected_output': '补充目标条目、现状/缺口与 file:line 的交付条件、禁止改文件等本任务排除项及覆盖声明要求；不复制整篇 spec 来替代指针，也不要求每次派发附上整套教学例子。'}, {'id': 'review-conformance-s25', 'prompt': 'S25 既有时效与失败规则继续生效\nGIVEN 派发涉及含 Superseded 条款的 active spec，且子代理执行失败\nWHEN 消费与恢复', 'expected_output': '派发保留现行条款过滤要求，失败按原单点缩小范围重试一次、再失败主线程接管；缩小处理批次不缩减总覆盖，未覆盖显式说明。'}])]
...
out=root/".spec-dev/2026-09-06-03-review-conformance/execution/serial/T03"
out.mkdir(parents=True,exist_ok=True)
phase="red"  # 首轮 red，改后另存 green；重试需新目录，禁止覆盖
files=['skills/requirement-analysis/references/exploration-patterns.md', 'skills/requirement-analysis/SKILL.md']
roles="\n\n".join("# "+p+"\n"+(root/p).read_text() for p in files)
snap={p:hashlib.sha256((root/p).read_bytes()).hexdigest() for p in files}
cases=[{'id': 'review-conformance-s24', 'prompt': 'S24 泛泛任务改成有界派发\nGIVEN 待派发内容只有「检查审查纪律」\nWHEN 按通用派发纪律准备输入', 'expected_output': '补充目标条目、现状/缺口与 file:line 的交付条件、禁止改文件等本任务排除项及覆盖声明要求；不复制整篇 spec 来替代指针，也不要求每次派发附上整套教学例子。'}, {'id': 'review-conformance-s25', 'prompt': 'S25 既有时效与失败规则继续生效\nGIVEN 派发涉及含 Superseded 条款的 active spec，且子代理执行失败\nWHEN 消费与恢复', 'expected_output': '派发保留现行条款过滤要求，失败按原单点缩小范围重试一次、再失败主线程接管；缩小处理批次不缩减总覆盖，未覆盖显式说明。'}]
cases=[c for c in cases if c["id"] in ['review-conformance-s24']]
for c in cases:
...
from pathlib import Path
changes = []
changes.append(('skills/requirement-analysis/references/exploration-patterns.md', r'''# 并行探索模式（阶段 2）
...
'''))
changes.append(('skills/requirement-analysis/SKILL.md', r'''**每个子代理必须给定**：清晰的主题或模态、相关文件线索、期望输出格式、工具优先级与文档时效规则提醒（后两项定义见 exploration-patterns 派发要求）。''', r'''**每个子代理必须给定**：有界主题、来源线索、可判定的完成条件、显式排除项、期望输出及适用的工具优先级/文档时效提醒；按 [exploration-patterns.md](references/exploration-patterns.md)「完成条件与排除项」和派发要求校准，不复制定义。'''))
changes.append(('skills/requirement-analysis/agents/openai.yaml', r'''  short_description: "需求设计：探索、逐题澄清、方案与完整设计批准；消费共享模块判据，公共测试落点随设计声明，spec 经独立审查后交接计划。"''', r'''  short_description: "需求设计：有界探索与明确交付条件、逐题澄清、方案和完整设计批准；消费共享判据，声明公共测试落点，spec 经独立审查后交接计划。"'''))
...
```bash
rtk git add skills/requirement-analysis/SKILL.md skills/requirement-analysis/agents/openai.yaml skills/requirement-analysis/evals/evals.json skills/requirement-analysis/references/exploration-patterns.md
rtk git add .spec-dev/2026-09-06-03-review-conformance/spec/review-conformance-design.md .spec-dev/2026-09-06-03-review-conformance/execution/serial/T03 .spec-dev/2026-09-06-03-review-conformance/plan/progress.yaml
rtk proxy node guardrail/check-spec-drift.mjs --staged
```

提交消息主旨为 `feat(T03): 通用派发完成条件`。以文件传入 Spec 指针和逐项共存说明，按实际命中的旧 spec 写 trailer；若无未知契约偏差，可用已批准的单次守卫豁免，不跳过 pre-commit。读取发版钩子后的最终 HEAD，再原子更新本票 completed/commit/tests；未通过必需验证保持未完成。
