...
- **相关测试范围**：计划头部声明的、与本特性改动相关的测试执行命令或清单。_Avoid_：测试分组、模块测试
...
writing-plans 生成计划时 SHALL 在计划头部产出「相关测试范围」节，内容为命令级测试执行声明，推导以项目测试影响分析工具优先、spec `covers` + 影响面 glob 兜底。
...
执行最终任务全量验证时，若失败测试位于相关测试范围之外，执行方 SHALL 回源分支复跑该测试并按结果分流：源分支同样失败则报告用户裁决是否阻塞合并，源分支通过则视为本次引入、修复后方可合并。
...
最终任务在全量验证通过后、合并前 SHALL 执行测试退役检查：扫描本特性 `covers` 触及的测试中的孤儿测试，列清单征询用户，同意后删除并计入提交；无孤儿测试时声明后跳过。
...
executing-plans SHALL 按计划声明的「相关测试范围」执行任务 0 与任务间验证，并在最终任务执行全量验证、归属裁决与退役检查；对缺该节的旧版计划 SHALL 按全量执行。（引用 writing-plans / using-git-worktrees 的判据，不复制。）
