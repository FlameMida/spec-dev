...
> 2026-09-10 历史状态补记：历史验收/审查报告中的 worktree 对象；test-scoping 另有 `8a1146e7` 分支合并记录。按用户裁决，以历史隔离工作区记录支持任务结果，空测试范围按原计划注明跳过；原始隔离检测过程记录缺失。详见[证据对账](../../reports/2026-09-10-01-legacy-plan-status-backfill.md)。本次未重新验收。
...
> 2026-09-10 历史状态补记：实施提交 `07760903` 的文件差异及历史验收报告。无；按最终交付和静态检查记录回填，不追认逐任务校验时序。详见[证据对账](../../reports/2026-09-10-01-legacy-plan-status-backfill.md)。本次未重新验收。
...
> 2026-09-10 历史状态补记：实施提交 `8fa9d11b` 的文件差异及历史验收报告。无；按最终交付和静态检查记录回填，不追认逐任务校验时序。详见[证据对账](../../reports/2026-09-10-01-legacy-plan-status-backfill.md)。本次未重新验收。

**文件**：
- 修改：`skills/writing-plans/SKILL.md:110-113`（任务 0 模板步骤 3）
- 修改：`skills/writing-plans/SKILL.md:187-224` 附近（「### 任务 N+2：合并与清理」模板块，已含子项目③的资源台账——必须保留）

**接口**：
- 消费：任务 1 的节名「相关测试范围」
- 产出：最终任务模板新步骤序（1 全量验证与归属裁决 → 2 测试退役检查 → 3 合并 → 4 台账清理 → 5 sync_commit 锚定；资源台账块与台账总则原样保留在步骤 1 之前）——任务 4 的 executing-plans 引用按此表述

- [x] **步骤 1：改写任务 0 模板步骤 3**
...
将「### 任务 N+2：合并与清理」markdown 围栏块整体替换为（围栏外的引导语与收尾说明段不动；**资源台账块与台账总则从现状原样保留**）：
...
> 2026-09-10 历史状态补记：实施提交 `ea86243e` 的文件差异及历史验收报告。无；按最终交付和静态检查记录回填，不追认逐任务校验时序。详见[证据对账](../../reports/2026-09-10-01-legacy-plan-status-backfill.md)。本次未重新验收。
...
> 2026-09-10 历史状态补记：实施提交 `77ccee1c` 的文件差异及历史验收报告。无；按最终交付和静态检查记录回填，不追认逐任务校验时序。详见[证据对账](../../reports/2026-09-10-01-legacy-plan-status-backfill.md)。本次未重新验收。

**文件**：
- 修改：`skills/executing-plans/SKILL.md:40`（阶段 2）
- 修改：`skills/executing-plans/SKILL.md:92`（阶段 6 第 2 点，③合并后已含台账清理措辞）
...
- [x] **步骤 2：阶段 6 第 2 点改引用**

将 L92 中"执行计划的最终任务（全量验证 → 合并回来源分支 → ……"的起始部分替换为"执行计划的最终任务（全量验证——范围外失败的归属裁决与测试退役检查按 writing-plans 最终任务模板执行 → 合并回来源分支 → ……"，其余（含台账清理与 sync_commit 锚定措辞）保持原文。
...
> 2026-09-10 历史状态补记：收尾提交 `bfa3857c`、历史验收报告、当前 worktree/分支/目录不存在的核对结果。无；清理按当前资源已不存在补记。详见[证据对账](../../reports/2026-09-10-01-legacy-plan-status-backfill.md)。本次未重新验收。

**资源台账**（清理依据）：

- [x] worktree: .worktrees/plan/2026-08-09-test-scoping —— `git worktree remove .worktrees/plan/2026-08-09-test-scoping && git branch -d plan/2026-08-09-test-scoping`
...
逐条执行资源台账各行的清理命令并勾选。命令执行失败 → 该行保留未勾选并报告用户；台账外一律不动。
任务 0 若经原生工具（EnterWorktree）建立 → 用原生方式退出，worktree 行按其机制收尾并注明。

- [x] **步骤 5：sync_commit 锚定**
