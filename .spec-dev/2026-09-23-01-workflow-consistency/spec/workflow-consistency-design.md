...
用户逐项选择：① final 全量前移；② 守卫使用任务范围核验；③ 未完成旧计划恢复时补齐。随后于 2026-09-23 对完整设计回复 `ok`，授权本稿及相关 ADR 保存、提交和独立审查；随后用户再次回复 `ok`，确认定稿并授权开始编写实施计划；尚未授权实施。
...
用户已确认定稿并授权编写计划，本 spec 于 2026-09-23 激活，同次对旧 spec 标明下列取代预告；真实交付时才逐条回写。旧文原句留作历史，后续消费者按新旧状态过滤。标题在回写时按原文完整匹配。
...
装机说明仍将 superseded 作为带 `superseded_by` 的生命周期终态：新工作沿后继读取，不依据已取代的行为规范实施；后继接管仍存在的代码 covers，旧 spec 的 sync_commit 冻结。不得通过改状态绕过守卫；本次只增加任务范围分支，不删除这些原义务。
...
引用继续绑定固定源码路径、行号及实际原文，并校验 findings schema；不存在/错误原文或目录外读取被拒绝。高/中候选只有不同于提出者的 actor 实际完成独立反驳后才可确认，主线程自查或覆盖摘要不能代替回执。控制器核验内容、动作与身份，语义判断仍归独立审查。
...
默认本地交付；PR 按已有工作流授权或仓库要求准备，一个特性复用一个 PR，各 push/create/ready/merge 动作仍核对对应授权。无实际差异不造空提交；授权缺失时完成本地准备后才询问具体发布动作。来源受保护仍在隔离分支实施，远端策略未知照实记录。PR open 时最终票 in_progress、roadmap in-progress、sync_commit 未锚定；真实合并及全部收尾完成后才回写 delivered。受保护目标上的文档回写需后续 PR 时保留未完成状态，不绕过保护或提前完成。
...
本稿已完成独立静态设计审查（Approved，见同目录 design-review.md）；用户确认定稿并授权编写计划，spec 已激活。没有新增 roadmap 子项目或批量修改旧 spec；后续 plan 与本 spec 共用本特性目录。原有 dirty 工作保持不动。
